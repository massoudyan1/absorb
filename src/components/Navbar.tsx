...
export function Navbar({ cartCount, onCartClick }: NavbarProps) {
  ...
  return (
    <nav className="fixed top-0 z-50 flex justify-between items-center w-full px-8 py-6 bg-black/80 backdrop-blur-md border-b-4 border-outline">
      <Link to="/" className="text-2xl font-black tracking-tighter text-white uppercase font-headline">
        absorb
      </Link>
      ...
      <div className="flex gap-6 items-center">
        <span className="font-headline uppercase tracking-[-0.02em] font-bold text-sm text-outline hidden lg:inline">STATUS: ONLINE</span>
        <button 
          onClick={onCartClick}
